...
\begin{equation}
	D\cdot\frac{u_{i+1} - 2\cdot u_i + u_{i-1}}{h^2} -k\cdot u = -s_i
\end{equation}

Nun wird nach der Variable $u_i$ sortiert und folgende Gleichung aufgestellt:
\begin{equation}
\frac{D}{h^2}\cdot u_{i-1} - \frac{2D}{h^2+k}\cdot u_i + \frac{D}{h^2}\cdot u_{i+1} = -s_i
\end{equation}
...
\begin{equation}
	u' = \frac{u_1-u_{-1}}{2h}, \quad u'(d) = \frac{u_{N+1}-u_{N-1}}{2h}
\end{equation}
