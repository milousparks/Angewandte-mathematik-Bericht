...
Im folgendem werden die Approximationen der Randbedigungen in die stationäre DGL (\cref{eq:stationDGL}) eingesetzt:
\begin{equation}
Z_N: \quad	D\cdot \frac{u_{N+1}-2u_n+u_{N-1}}{h^2}-ku_N=-s_N
\end{equation}
mit der Approximation \cref{eq:approx_RN} folgt:

	\begin{equation}
		\begin{split}
				Z_N: \quad	D\cdot \frac{\frac{-2\cdot h\cdot S_R}{D} \cdot u_\mathrm{N} + u_{\mathrm{N}-1}-2u_n+u_{N-1}}{h^2}-ku_N=-s_N \\
					\frac{-2hS_R-2D}{h^2}\cdot u_\mathrm{N}+\frac{2D}{h^2}\cdot u_{N-1}-ku_\mathrm{N}=-s_\mathrm{N}\\
						\frac{-2hS_R-2D-kh^2}{h^2}\cdot u_\mathrm{N}+\frac{2D}{h^2}\cdot u_{N-1}=-s_\mathrm{N}
		\end{split}
	\end{equation}
...
\begin{qed}
	...
	\begin{equation}
	\tcbhighmath{Z_N: \quad	\frac{-2hS_R-2D-kh^2}{h^2}\cdot u_\mathrm{N}+\frac{2D}{h^2}\cdot u_{N-1}=-s_\mathrm{N}}
	\end{equation}
\end{qed}
