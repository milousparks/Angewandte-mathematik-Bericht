...
\begin{equation}
	 D\cdot F_{\mathrm{euler}}(z) = D\cdot z - h\cdot D_(y)\cdot f(t_i+1, y^(i) + z) 
\end{equation}


für das implizite Euler-Verfahren und
\begin{equation}
	 D\cdot F_{\mathrm{euler}}(z) = \frac{h}{2}\cdot \left[D_{(y)}\cdot f\left(t_i+1, y^(i)) +  h\cdot D_(y)f(t_i+1, y^{(i)} + z)\right)\right] - D\cdot z 
\end{equation}
